...
def Cesar(
    ...
    if e or d:
        ...
        if file:
            msg = fileRead(text)
            fileWrite(newFilePath(text, mode), CesarCipher(msg, key, mode, space))
        else:
...
def Vigenere(
    ...
    space: bool = Option(False, help="with space (obs Only with --e)"),
    text: str = Argument(..., help="the msg to encrypt or decrypt"),
    key: str = Argument(..., help="the key to encrypt or decrypt"),
):
    if e or d:
        if e:
            mode = "e"
        ...
        if file:
            msg = fileRead(text)
            fileWrite(newFilePath(text, mode), Vigenere(msg, key, mode, space))
        else:
            print(Vigenere(text, key, mode, space))
    else:
        print("Information needed flags --e or --d")
